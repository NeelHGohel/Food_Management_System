...
function Gujrati() {
    ...
    return (
        <div className="container">
            <h1 className="text-center my-4">Delicious Gujrati Dishes</h1>
            <div className="row">
                {data.length > 0 ? (
                    data.map((guj) => (
                        <div className="col-md-4 col-sm-6 my-2" key={guj.FacultyID}>
                            <div className="card shadow-lg border-light transition">
                                <img
                                    src={guj.FacultyImage}
                                    alt={guj.FacultyName}
                                    className="card-img-top dish-image"
                                />
                                <div className="card-body text-center">
                                    <h5 className="card-title font-weight-bold">{guj.FacultyName}</h5>
                                    <p className="card-text text-muted">ID: {guj.FacultyID}</p>
                                    <button
                                        className="btn btn-danger"
                                        onClick={() => handleAddToCart(guj)}
                                    >
                                        Add to Cart
                                    </button>
                                </div>
